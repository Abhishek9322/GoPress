...
namespace GoPress.Infrastructure.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        //    var allShops = await _context.ApplicationUsers
        //        .Include(x => x.ShopOwnerProfile)
        //        .Where(x => x.Role == UserRoleenum.ShopOwner)
        //        .ToListAsync();
        ...
        //    Console.WriteLine($"Role : {await query.CountAsync()}");

        //    query = query.Where(x => x.IsApproved);

        //    Console.WriteLine($"Approved : {await query.CountAsync()}");

        //    query = query.Where(x => x.IsActive);

        //    Console.WriteLine($"Active : {await query.CountAsync()}");

        //    query = query.Where(x => x.ShopOwnerProfile != null);

        //    Console.WriteLine($"Profile : {await query.CountAsync()}");

        //    query = query.Where(x =>
        //        x.ShopOwnerProfile!.City.Trim().ToLower() ==
        //        city.Trim().ToLower());

        //    Console.WriteLine($"City : {await query.CountAsync()}");

        //    query = query.Where(x => x.ShopOwnerProfile!.IsOpen);

        //    Console.WriteLine($"Open : {await query.CountAsync()}");

        //    query = query.Where(x =>
        //        currentTime >= x.ShopOwnerProfile!.OpeningTime &&
        //        currentTime <= x.ShopOwnerProfile.ClosingTime);

        //    Console.WriteLine($"Time : {await query.CountAsync()}");
